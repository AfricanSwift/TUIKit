...
public struct Example_LineGraphics
{
  public static func demo()
  {
    ...
    for s in stride(from: 1.0, to: 5.0, by: 0.1)
    {
      
      let rect1 = TUIRectangle(origin: TUIPoint(x: 0, y: 0),
                               size: TUISize(width: 100, height: 100))
      let rect2 = TUIRectangle(origin: TUIPoint(x: 16, y: 16),
                               size: TUISize(width: 70, height: 70))
      ...
      view.drawStar(center: TUIPoint(x: 50, y: 50), radius1: 48, radius2: 10, segments: s)
